...
#' 
#' @description Generate multiple visualizations and perform statistical test to detect major lineages and 
...
#'    `output_dir` in the latter function (default: NULL)
#' @param pal_lineages Customisable palette to use for major_lineage plots. Specified as c("lineage_label"="#color", ...)
#' @param out_folder Output folder which will be appended to `stat_results/` in the working directory to 
#'    generate results for multiple thresholds. E.g. `out_folder=="period2"` then results will go to
#'    `stat_results/period2/threshold_quantile_0.25`, and the other 9 thresholds (0.5, 0.75, 1, 2, 3, 
#'    4, 5, 10, 25). Default: NULL
#' @param out_poisson_file Output file for poisson regressions across thresholds having the Freq of homoplasy
#'    as response and major lineage, genomic region and whether independently found under positive selection 
#'    as predictors. Will be appended to `stat_results/{out_folder}/`. E.g. assuming again `out_folder=="period2"`
#'    and now `out_poisson_file=="poisson_p2.txt"` then output will be added to `stat_results/period2/poisson_p2.txt`
#'
#' @return A list with 3 elements: 
#'    * `bubble_non_syn_clust1` gives TFP-homoplasy frequency bubble plots across thresholds
#'    for each major lineage and intersection with positive selection
#'    * `s9` returns as the first nested list the positioning and frequency of non-synonymous TFP-homoplasies for each genomic
...
#'    * `s13` returns the same as `s9` but for synonymous instead of non-synonymous TFP-homoplasies
#' @export
stats_multiple_thresholds <- function(path_stats, pal_lineages = c("Alpha_B.1.1.7"="#fd7f6f", "Delta_AY.4.*"="#7eb0d5", "Delta_other"="#b2e061", "EU1_B.1.177"="#bd7ebe", "Omicron_BA.1.*"="#ffb55a", "Omicron_BA.2.*"="#ffcc00", "Other"="#fdcce5"), out_folder, out_poisson_file) {
	...
	.total_amount_unique_homoplasies <- function(df, mut_type) {
		...
		total_homopls_region_higher_thr <- df_freq_genomic_regions_first %>% dplyr::filter(threshold>=2) %>% dplyr::group_by(spec_regions) %>% dplyr::summarise(total=dplyr::n())
...
#' Plot boxplot of the clustering statistics
#'
#' @description Plot boxplot of the clustering stats values across clades detected by at least one of the stats (union) for different thresholds
#' 
#' @inheritParams stats_multiple_thresholds
#' @param ymin_vec Manual adjustment of minimum values for (i) ratio sizes, (ii) ratio persistence time,
#'    and (iii) logistic growth rate
#' @param ymax_vec Same as above but adjusting for maximum values
#'
#' @return An arranged figure with 3 rows (one for each statistic) containing boxplots of stat values for each threshold
#' @export
...
#' Plot the number of TFP-homoplasies by genomic region coloured by the first threshold detected
#'
#' @param path Directory path of the form `stat_results/{out_folder}/genomewide_plot_non-syn/` obtained 
#'    after running [stats_multiple_thresholds()]
#'
#' @return Plot TFP-homoplasy sites by genomic region coloured by the first threshold detected. 
#'    This is also generated at `{path}/stacked_nsites_threshold/`
#' @export
stacked_nsites_genomic_region_mult_thresholds <- function(path) {
	...
	p <- ggplot2::ggplot(reg_thr_files_df, ggplot2::aes(fill=threshold, x=n, y=spec_regions)) + 
		...
		ggplot2::labs(y="Genomic region", x="Number of identified TFP-homoplasies")
